...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array SfGuardProfile[] Collection to store aggregation of SfGuardProfile objects.
	 */
	protected $collSfGuardProfiles;

	/**
	 * @var        Criteria The criteria used to select the current contents of collSfGuardProfiles.
	 */
	private $lastSfGuardProfileCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collSfGuardProfiles = null;
			$this->lastSfGuardProfileCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collSfGuardProfiles !== null) {
				foreach ($this->collSfGuardProfiles as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getSfGuardProfiles() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addSfGuardProfile($relObj->copy($deepCopy));
				}
			}
	 ...
	 * related AlpzaMiembroAsignados from storage. If this sfGuardUser is new, it will return
	...
	public function getAlpzaMiembroAsignados($criteria = null, PropelPDO $con = null)
	{
		...
		if ($this->collAlpzaMiembroAsignados === null) {
			if ($this->isNew()) {
				...
				$criteria->add(AlpzaMiembroAsignadoPeer::ID_SF_GUARD_USER, $this->id);
	 ...
	 */
	public function getAlpzaMiembroAsignadosJoinAlpzaMiembro($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collAlpzaMiembroAsignados === null) {
			if ($this->isNew()) {
				...
				$this->collAlpzaMiembroAsignados = AlpzaMiembroAsignadoPeer::doSelectJoinAlpzaMiembro($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collSfGuardProfiles collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addSfGuardProfiles()
	 */
	public function clearSfGuardProfiles()
	{
		$this->collSfGuardProfiles = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collSfGuardProfiles collection (array).
	 *
	 * By default this just sets the collSfGuardProfiles collection to an empty array (like clearcollSfGuardProfiles());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initSfGuardProfiles()
	{
		$this->collSfGuardProfiles = array();
	}

	/**
	 * Gets an array of SfGuardProfile objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related SfGuardProfiles from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array SfGuardProfile[]
	 * @throws     PropelException
	 */
	public function getSfGuardProfiles($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collSfGuardProfiles === null) {
			if ($this->isNew()) {
				...
				$criteria->add(SfGuardProfilePeer::ID_SF_GUARD_USER, $this->id);

				SfGuardProfilePeer::addSelectColumns($criteria);
				$this->collSfGuardProfiles = SfGuardProfilePeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				SfGuardProfilePeer::addSelectColumns($criteria);
				if (!isset($this->lastSfGuardProfileCriteria) || !$this->lastSfGuardProfileCriteria->equals($criteria)) {
					$this->collSfGuardProfiles = SfGuardProfilePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastSfGuardProfileCriteria = $criteria;
		return $this->collSfGuardProfiles;
	}

	/**
	 * Returns the number of related SfGuardProfile objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related SfGuardProfile objects.
	 * @throws     PropelException
	 */
	public function countSfGuardProfiles(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collSfGuardProfiles === null) {
			if ($this->isNew()) {
				...
				$count = SfGuardProfilePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastSfGuardProfileCriteria) || !$this->lastSfGuardProfileCriteria->equals($criteria)) {
					...
					$count = count($this->collSfGuardProfiles);
				}
			} else {
				$count = count($this->collSfGuardProfiles);
			}
	...
	/**
	 * Method called to associate a SfGuardProfile object to this object
	 * through the SfGuardProfile foreign key attribute.
	 *
	 * @param      SfGuardProfile $l SfGuardProfile
	 * @return     void
	 * @throws     PropelException
	 */
	public function addSfGuardProfile(SfGuardProfile $l)
	{
		if ($this->collSfGuardProfiles === null) {
			$this->initSfGuardProfiles();
		}
		if (!in_array($l, $this->collSfGuardProfiles, true)) { // only add it if the **same** object is not already associated
			array_push($this->collSfGuardProfiles, $l);
			$l->setsfGuardUser($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collSfGuardProfiles) {
				foreach ((array) $this->collSfGuardProfiles as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
