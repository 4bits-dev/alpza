...
/**
 * Base class that represents a row from the 'alpza_noticia' table.
 *
 *
 *
 ...
 */
abstract class BaseAlpzaNoticia extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the noticia field.
	 * @var        string
	 */
	protected $noticia;
	...
	/**
	 * Initializes internal state of BaseAlpzaNoticia object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     AlpzaNoticia The current object (for fluent API support)
	 */
	public function setIdAlpzaNoticia($v)
	{
		...
		if ($this->id_alpza_noticia !== $v) {
			...
			$this->modifiedColumns[] = AlpzaNoticiaPeer::ID_ALPZA_NOTICIA;
		}
	...
	public function setAlpzaMiembroIdAlpzaMiembro($v)
	{
		...
		if ($this->alpza_miembro_id_alpza_miembro !== $v) {
			...
			$this->modifiedColumns[] = AlpzaNoticiaPeer::ALPZA_MIEMBRO_ID_ALPZA_MIEMBRO;
		}
	...
	public function setNombre($v)
	{
		...
		if ($this->nombre !== $v) {
			$this->nombre = $v;
			$this->modifiedColumns[] = AlpzaNoticiaPeer::NOMBRE;
		}
	...
	public function setExtracto($v)
	{
		...
		if ($this->extracto !== $v) {
			$this->extracto = $v;
			$this->modifiedColumns[] = AlpzaNoticiaPeer::EXTRACTO;
		}
	...
	/**
	 * Set the value of [noticia] column.
	 *
	 * @param      string $v new value
	 * @return     AlpzaNoticia The current object (for fluent API support)
	 */
	public function setNoticia($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
		...
		if ($this->noticia !== $v) {
			$this->noticia = $v;
			$this->modifiedColumns[] = AlpzaNoticiaPeer::NOTICIA;
		}
	...
	public function setEstado($v)
	{
		...
		if ($this->estado !== $v || $this->isNew()) {
			$this->estado = $v;
			$this->modifiedColumns[] = AlpzaNoticiaPeer::ESTADO;
		}
	...
	public function setFechaIngreso($v)
	{
		...
		if ( $this->fecha_ingreso !== null || $dt !== null ) {
			...
			)
			{
				...
				$this->modifiedColumns[] = AlpzaNoticiaPeer::FECHA_INGRESO;
			}
	...
	public function setFechaEdicion($v)
	{
		...
		if ( $this->fecha_edicion !== null || $dt !== null ) {
			...
			)
			{
				...
				$this->modifiedColumns[] = AlpzaNoticiaPeer::FECHA_EDICION;
			}
		} // if either are not null
	...
	public function setFechaPublicacion($v)
	{
		...
		if ( $this->fecha_publicacion !== null || $dt !== null ) {
			...
			)
			{
				...
				$this->modifiedColumns[] = AlpzaNoticiaPeer::FECHA_PUBLICACION;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_alpza_noticia = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			...
			$this->noticia = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			...
			return $startcol + 9; // 9 = AlpzaNoticiaPeer::NUM_COLUMNS - AlpzaNoticiaPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating AlpzaNoticia object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AlpzaNoticiaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseAlpzaNoticia:delete:pre') as $callable)
			{
			...
			if ($ret) {
				AlpzaNoticiaPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseAlpzaNoticia:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseAlpzaNoticia:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseAlpzaNoticia:save:post') as $callable)
				{
					call_user_func($callable, $this, $con, $affectedRows);
				}

				AlpzaNoticiaPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = AlpzaNoticiaPeer::doInsert($this, $con);
					$affectedRows += 1; // we are assuming that there is only 1 row per doInsert() which
					// should always be true here (even though technically
					// BasePeer::doInsert() can insert multiple rows).

					$this->setIdAlpzaNoticia($pk);  //[IMV] update autoincrement primary key

					$this->setNew(false);
				} else {
					$affectedRows += AlpzaNoticiaPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AlpzaNoticiaPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AlpzaNoticiaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = AlpzaNoticiaPeer::getFieldNames($keyType);
		$result = array(
		$keys[0] => $this->getIdAlpzaNoticia(),
		$keys[1] => $this->getAlpzaMiembroIdAlpzaMiembro(),
		$keys[2] => $this->getNombre(),
		$keys[3] => $this->getExtracto(),
		$keys[4] => $this->getNoticia(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIdAlpzaNoticia($arr[$keys[0]]);
		...
		if (array_key_exists($keys[4], $arr)) $this->setNoticia($arr[$keys[4]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AlpzaNoticiaPeer::ID_ALPZA_NOTICIA)) $criteria->add(AlpzaNoticiaPeer::ID_ALPZA_NOTICIA, $this->id_alpza_noticia);
		if ($this->isColumnModified(AlpzaNoticiaPeer::ALPZA_MIEMBRO_ID_ALPZA_MIEMBRO)) $criteria->add(AlpzaNoticiaPeer::ALPZA_MIEMBRO_ID_ALPZA_MIEMBRO, $this->alpza_miembro_id_alpza_miembro);
		if ($this->isColumnModified(AlpzaNoticiaPeer::NOMBRE)) $criteria->add(AlpzaNoticiaPeer::NOMBRE, $this->nombre);
		if ($this->isColumnModified(AlpzaNoticiaPeer::EXTRACTO)) $criteria->add(AlpzaNoticiaPeer::EXTRACTO, $this->extracto);
		if ($this->isColumnModified(AlpzaNoticiaPeer::NOTICIA)) $criteria->add(AlpzaNoticiaPeer::NOTICIA, $this->noticia);
		if ($this->isColumnModified(AlpzaNoticiaPeer::ESTADO)) $criteria->add(AlpzaNoticiaPeer::ESTADO, $this->estado);
		if ($this->isColumnModified(AlpzaNoticiaPeer::FECHA_INGRESO)) $criteria->add(AlpzaNoticiaPeer::FECHA_INGRESO, $this->fecha_ingreso);
		if ($this->isColumnModified(AlpzaNoticiaPeer::FECHA_EDICION)) $criteria->add(AlpzaNoticiaPeer::FECHA_EDICION, $this->fecha_edicion);
		if ($this->isColumnModified(AlpzaNoticiaPeer::FECHA_PUBLICACION)) $criteria->add(AlpzaNoticiaPeer::FECHA_PUBLICACION, $this->fecha_publicacion);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(AlpzaNoticiaPeer::ID_ALPZA_NOTICIA, $this->id_alpza_noticia);
	...
	/**
	 * Generic method to set the primary key (id_alpza_noticia column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of AlpzaNoticia (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setNoticia($this->noticia);
		...
		$copyObj->setIdAlpzaNoticia(NULL); // this is a auto-increment column, so set to default value
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     AlpzaNoticia Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     AlpzaNoticiaPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new AlpzaNoticiaPeer();
		}
		return self::$peer;
	}
	...
	public function getAlpzaMiembro(PropelPDO $con = null)
	{
		if ($this->aAlpzaMiembro === null && ($this->alpza_miembro_id_alpza_miembro !== null)) {
			...
			/* The following can be used additionally to
			 ...
			 $this->aAlpzaMiembro->addAlpzaNoticias($this);
			 */
		}
	...
	{
		if (!$callable = sfMixer::getCallable('BaseAlpzaNoticia:'.$method))
		{
			throw new sfException(sprintf('Call to undefined method BaseAlpzaNoticia::%s', $method));
		}
